...
class CommandsGroup extends parent.CommandsHolderClass
{
  ...
  trigger(context)
  {
    ...
    {
      var raw_args = context.content.toLowerCase().replace(this.prefix, "");
      let args = split_args(raw_args);
      this.commands.forEach(command =>
        {
        if (context.content.startsWith(this.prefix) && (args[0] == command.keyword || command.alias.indexOf(args[0]) != -1))
        {
